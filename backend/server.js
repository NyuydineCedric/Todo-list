...
import { sendReminderEmail } from './utils/emailService.js'; // uses Brevo via axios
...
app.post('/api/auth/signup', async (req, res) => {
  ...
  const user = { id: generateId(), email, passwordHash: hashed, name: name || '', createdAt: new Date().toISOString() };
  addUser(user);
...
const runScheduler = async (catchAll = false) => {
  ...
  for (const task of dueReminders) {
    ...
    if (!user?.email) {
      console.warn(`⚠️ No email for user ${task.userId}, skipping`);
      continue;
    }
    const success = await sendReminderEmail(user.email, task.title, task.description, task.dueDate);
    if (success) {
      addEmailLog({
        id: generateId(),
        taskId: task.id,
      ...
      updateTask(task.id, task.userId, { reminderTime: null });
...
app.listen(PORT, () => {
  ...
  runScheduler(true); // catch‑up run on startup
